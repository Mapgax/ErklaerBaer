...
- **Check speech against its text before rendering.** Characters per voiced second and the count of separated bursts catch a repeated or padded take in one pass, and rendering is the expensive step.
...
For cached review reproduction use `.venv/bin/python3 scripts/build_v3_guitar.py --expressive --cache-only`. A cache miss must fail. Bookends come from `scripts/build_brand_intro.py --duration 6 --outro-duration 4`. Upload with `scripts/upload_v3_review.py`, which waits for processing and keys its marker on `episode|cut`; `DRY_RUN=false` belongs on the invocation, never in `.env`. Paid promotion and Shorts remix are not exposed by the Data API and are set in Studio after each upload. Consult the recovery guide before any new call or restore.
